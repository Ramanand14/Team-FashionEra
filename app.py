    ...
    try:
        ...
        with conn:
            ...
            if request.method=='POST':
                ...
                print("Password: ", PWD)
                sql = '''insert into UserData(ID,UserName,Email,Cell,NickName,PWD) values(?,?,?,?,?,?)'''
                cur = conn.cursor()
                data = (ID,UserName,Email,Cell,NickName,PWD)
                cur.execute(sql,data)
                conn.commit()
                rows=0
                cur.execute("Select * from UserDate where UserName = ? and Email = ? and PWD = ?",(ID,UserName,Email,Cell,NickName,PWD,))
                rows = cur.fetchall()
                for row in rows:
